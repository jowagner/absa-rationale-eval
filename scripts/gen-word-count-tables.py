...
for block_se, nse in [
    ...
    for block_r, nr in [
        ...
        tokens = cell2table[(block_se,block_r)]['ordered tokens']
        wcount = 0
        for token in tokens:
            wcount += cell2table[(block_se,block_r)][token][0]
        total = len(tokens)
        top_10 = tokens[:10]
        sys.stdout.write('\\hline\n')
        sys.stdout.write(r'\multicolumn{9}{l}{Top 10 \textbf{%sSE} $\cap$ \textbf{%sR} words (%d types, %d tokens)}\\' %(nse, nr, total, wcount))
        sys.stdout.write('\n\\hline\n')
        for token in top_10:
            ...
            for column_se in 'io':
                for column_r in 'io':
                    try:
                        ...
                        freq = 0
                        rank = ' -- '
                    sys.stdout.write('& %7.1f & %s ' %(freq/9.0, rank))
            sys.stdout.write('\\\\\n')


sys.stdout.write(r"""    \hline
        \end{tabular}
    \caption{Average frequency (over nine runs) and rank of top 10 words in each word cloud of Table 2}
